...
def test_2flow(tol=5e-12, tol2=1e-7, tol3=1e-5):
    ...
    t1 = 1
    diff1 = Hflow(testpl1, t=t1) - testop(testpl1, t=t1)
    check1 = max(np.abs(list(diff1.values())))
    assert check1 < tol, f'{check1} >= {tol}' 
    
    # check linearity and multiplicity
    diff2 = Hflow(testpl1) - Hflow(f[10]) - f[11]*Hflow(xi1) - f[12]*Hflow(xi2) - f[13]*Hflow(eta1) - f[14]*Hflow(eta2) - \
    ...
    check2 = max(np.abs(list(diff2.values())))
    assert check2 < tol, f'{check2} >= {tol}'
    
    # check interoperability with commutator;
    # first check:
    t3 = 3.6
    diff3 = Hflow(testpl3, t=t3)@Hflow(testpl4, t=t3) - Hflow(testpl3@testpl4, t=t3)
    check3 = max(np.abs(list(diff3.values())))
    assert check3 < tol2, f'{check3} >= {tol2}' 

    # second check:
    t4 = 3.6
    diff4 = Hflow(testpl3, t=t4)*Hflow(testpl4, t=t4) - Hflow(testpl3*testpl4, t=t4)
    check4 = max(np.abs(list(diff4.values())))
    assert check4 < tol3, f'{check4} >= {tol3}' 
    
    # third check:
    t5 = 0.6
    diff5 = Hflow(testpl1, t=t5)@Hflow(testpl2, t=t5) - Hflow(testpl1@testpl2, t=t5)
    check5 = max(np.abs(list(diff5.values())))
    assert check5 < tol, f'{check5} >= {tol}' 
    
    # forth check (against ref):
    t6 = t5
    diff6 = Hflow(testpl1, t=t6)@Hflow(testpl2, t=t6) - testop(testpl1@testpl2, t=t6)
    check6 = max(np.abs(list(diff6.values())))
